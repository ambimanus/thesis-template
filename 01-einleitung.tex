...
\begin{table}
  ...
  \begin{tabular}{@{}llAA@{}}  % "@{}" entfernt margins links und rechts
    ...
    linear & lin-lin & y &= \sbeta[0] + \sbeta[1] \cdot x & \hat{y} &= \sbeta[0] + \sbeta[0] \cdot x \\
    quadratisch & sqrt-lin & \sqrt{y} &= \sbeta[0] + \sbeta[1] \cdot x & \hat{y} &= (\sbeta[0] + \sbeta[0] \cdot x)^2 \\
    exponentiell & log-lin & \ln y &= \sbeta[0] + \sbeta[1] \cdot x & \hat{y} &= \mathrm{e}^{\sbeta[0] + \sbeta[0] \cdot x} \\
    \bottomrule
  \end{tabular}
